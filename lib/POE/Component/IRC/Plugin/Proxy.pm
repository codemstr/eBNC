...
sub _client_input {
    ...
    if ($input->{command} eq 'USRIP') {
	#$irc->send_event(irc_proxy_authed => $wheel_id);
         print "Got USRIP request from $wheel_id ". $wheels->{$wheel_id}{peer} . "\n";
	#$self->_send_to_client($wheel_id, "CHALLENGE 96cf6487");
        $self->_send_to_client($wheel_id, ':sponji.wtf.im 302 unknown=+unknown@' . $wheels->{$wheel_id}{peer});
        # $self->_send_to_client($wheel_id, ':sponji.wtf.im 302 unknown=+unknown@' . $wheels->{$wheel_id}{peer} );
    ...
    if ($input->{command} =~ /^(?:USRIP|NICK|USER|PASS)$/) {
        return;
    }
